...
def count_antinodes_2(input_str):
    ...
    for i in positions:
        for j in range (len(positions[i])):
            for k in range(j + 1, len(positions[i])):
                antinodes_temp = antinodes_position_2(positions[i][j], positions[i][k], n, m)
...
def antinodes_positions(pos1, pos2, n, m):
    ...
    if (antinode_pos1[0] < 0 or antinode_pos1[0] >= n or antinode_pos1[1] < 0 or antinode_pos1[1] >= m) and (antinode_pos2[0] < 0 or antinode_pos2[0] >= n or antinode_pos2[1] < 0 or antinode_pos2[1] >= m):
        print("0", antinode_pos1, antinode_pos2)
        return None
    
    elif antinode_pos1[0] < 0 or antinode_pos1[0] >= n or antinode_pos1[1] < 0 or antinode_pos1[1] >= m:
        return antinode_pos2

    elif antinode_pos2[0] < 0 or antinode_pos2[0] >= n or antinode_pos2[1] < 0 or antinode_pos2[1] >= m:
